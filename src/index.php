...
function get_variant ($agent, $appid, $time, $user)
{
	...
	if ($user !== 0)
	{
		...
		$matches = array
		(
			'drunk'				=> function ($state) { return $state->month === 1 && $state->day === 1; },
			'crown'				=> function ($state) { return $state->month === 1 && $state->day === 6; },
			'yoshi'				=> function ($state) { return $state->month === 1 && $state->day === 24; },
			'pancake'			=> function ($state) { return $state->month === 2 && $state->day === 2; },
			'valentine'			=> function ($state) { return $state->month === 2 && $state->day === 14; },
			'cardboard'			=> function ($state) { return $state->month === 2 && $state->day === 17; },
			'bear'				=> function ($state) { return $state->month === 6 && $state->day === 25; },
			'redhat'			=> function ($state) { return $state->month === 7 && $state->day === 1; },
			'fireworks'			=> function ($state) { return $state->month === 7 && $state->day === 14; },
			'snake'				=> function ($state) { return $state->month === 8 && $state->day === 2; },
			'china'				=> function ($state) { return $state->month === 8 && $state->day === 29; },
			'unicorn'			=> function ($state) { return $state->month === 9 && $state->day === 9; },
			'penguin'			=> function ($state) { return $state->month === 9 && $state->day === 12; },
			'space'				=> function ($state) { return $state->month === 9 && $state->day === 27; },
			'particule'			=> function ($state) { return $state->month === 10 && $state->day === 15; },
			'swan'				=> function ($state) { return $state->month === 10 && $state->day === 18; },
			'pizza'				=> function ($state) { return $state->month === 10 && $state->day === 21; },
			'pixel'				=> function ($state) { return $state->month === 10 && $state->day === 23; },
			'ghost'				=> function ($state) { return $state->month === 10 && $state->day === 31; },
			'totoro'			=> function ($state) { return $state->month === 11 && $state->day === 4; },
			'cornflower'		=> function ($state) { return $state->month === 11 && $state->day === 11; },
			'kirby'				=> function ($state) { return $state->month === 12 && $state->day === 19; },
			'christmas-dark'	=> function ($state) { return $state->month === 12 && $state->day < 24 && ($state->hour + 2) % 24 <= 7; },
			'christmas'			=> function ($state) { return $state->month === 12 && $state->day < 24; },
			'santa'				=> function ($state) { return $state->month === 12 && $state->day === 24; },
			'gift'				=> function ($state) { return $state->month === 12 && $state->day === 25; },
			'cola'				=> function ($state) { return $state->month === 12 && $state->day === 28; },
			'yellow'			=> function ($state) { return $state->month === 12 && $state->day < 31; },
			'party'				=> function ($state) { return $state->month === 12 && $state->day === 31; },
			'dark'				=> function ($state) { return $state->hour === 3 && $state->minute === 0; },
			'sleep'				=> function ($state) { return ($state->hour + 1) % 24 <= 7; },
			'freeze'			=> function ($state) { return $state->temperature < 269.15; },
			'bobblehat'			=> function ($state) { return $state->temperature < 277.15; },
			'hot'				=> function ($state) { return $state->temperature > 305.15; },
			'umbrella'			=> function ($state) { return $state->condition === 'rain' && $state->weather > 500 && $state->cloudiness >= 50; },
			'snow'				=> function ($state) { return $state->condition === 'snow' && $state->weather > 600 && $state->cloudiness >= 50; },
			'sunglass'			=> function ($state) { return $state->hour >= 10 && $state->hour <= 18 && $state->cloudiness < 10 && $state->temperature > 297.15; }
		);
